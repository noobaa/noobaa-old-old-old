...
function verify_and_create_base_folder(folder_name, user, next) {
    ...
    Inode.findOne({
        ...
        if (err) {
            console.error('ERROR find basic folder:', folder_name,
                'user:', user.get_name(), err);
        ...
        if (!inode) {
            ...
            return inode.save(function(err, inode, num) {
                ...
                console.log('Created basic folder:', folder_name,
                    'id:', inode._id, 'user:', user.get_name());
                return next(null, user);
            });
        }
        // console.log('inode exists. File name:', folder_name, user.get_name());
        return next(null, user);
    });
}
